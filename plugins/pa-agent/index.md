...
- Dagstart request — the opening-the-day moment, including a plain "goedemorgen" (that greeting itself is the trigger, not just an explicit "start mijn dagstart" — it's typically the first thing that happens in a session) → skill `.claude/skills/dagstart/SKILL.md`; its personal calibration lives in `personal/dagstart.md`. Currently a thin wrapper around `agenda-briefing`, but the moment is meant to grow into much more (mail, taken, context).
