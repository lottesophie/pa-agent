...
- Type: client, colleague, invoice, newsletter, notification, personal message, system/security alert. Treat this list as a starting point, not a closed set — if a stable new type keeps showing up, that is a candidate for `personal/email-triage.md`, not for silently forcing it into an existing category.

## Step 3 — Determine urgency

- Signal words ("urgent", "deadline", "final notice", "ASAP"), an explicit deadline in the text, escalation pattern (repeated message on the same topic without a reply = rising urgency).
...
- Group by classification (action-required/urgent first), scannable.
...
Known preference topics to listen for (checklist, not a form to fill): VIP list / always-low senders, category definitions (what counts as business vs. personal, which account), label/folder names, autonomy boundaries, tone & language per context, rhythm/timing (how often triage runs, quiet hours), thresholds (after how many days a thread counts as "stale", sensitivity to false positives vs. false negatives), exceptions (newsletters that are wanted anyway, contacts that must never be missed).
